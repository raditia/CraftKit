...
The ask is underspecified and guessing is expensive: a new feature/screen/module with unstated scope, users, constraints, or success criteria. If the ask is already clear, skip and go straight to `/spec` or `/parallel-build`.
...
3. **Prefer concrete over open.** Offer 2–4 candidate answers when you can (use `AskUserQuestion`) and mark which one you recommend, since recognition beats recall and a recommendation gives the user something to push against. Fall back to open text only when the space is genuinely unbounded.
...
Skip a rank if the ask already answers it, and spend questions only on unknowns.
