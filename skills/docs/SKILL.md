...
> **Core behaviors:** Same facts, two registers, and never let the audiences disagree. Document the *why*, not just the what. See `/using-agent-skills`.
...
- Every stakeholder claim traces to a real technical fact, with no aspirational capability.
...
- **Present** → invoke `/humanizer` on each doc. Instruct it to preserve markdown structure verbatim, so headings, tables, and inline `code` pass through untouched. Use the humanized output as final.
...
Both documents are **plain markdown meant to be pasted into Confluence** (via the editor's `/Markdown` insert or the `+` → *Markdown* option). Write markdown that survives that paste, sticking to constructs Confluence maps cleanly:
...
| Fenced code blocks with a language (```ts) | Indented (4-space) code blocks, which become paragraphs |
| Standard `-` bullets, `1.` ordered lists | Deep nesting (> 3 levels), which flattens |
