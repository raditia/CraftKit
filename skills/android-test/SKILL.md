...
> **Core behaviors:** The unit under test is the **Presenter** (or a `StateFlow` ViewModel), never the Activity/Fragment. Tests must fail if the business rule changes, not just exercise getters. Never skip verification, since tests must pass before done. See `/using-agent-skills` and `/android-patterns`.
...
### StateFlow ViewModel (newer screens) with Turbine
...
When the feature has test cases (`test-cases-resolve` above), every approved case with Automation `junit` gets exactly one test, titled with its ID: `` @Test fun `TC-003 …`() ``. List `manual` cases in the report instead of testing them. Coverage of the diff below still applies on top.
