...
> **Core behaviors:** Surface assumptions about what needs testing. Never skip verification, since tests must pass and coverage must be ≥ 93% before done. See `/using-agent-skills`.
...
Mock all hooks from shared packages. Fluent tracker chaining (`track(...).send(...)`) requires `send` on the inner mock return, since a mismatch causes `TypeError`:
```ts
useTracker: jest.fn(() => jest.fn(() => ({ send: jest.fn() }))),
```

### Network mocking
Use MSW for fetch/React Query flows. Set `onUnhandledRequest: 'error'`, because silent passes mask missing mocks.
...
- `findBy*` is an async Promise (elements appearing after async work)
...
- State transitions (initial → after action), wrapped in `act`
...
```
Never `setTimeout` + assertion, which is flaky.
