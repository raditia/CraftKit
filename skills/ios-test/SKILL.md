...
final class <Prefix>ViewModelTest: QuickSpec {
    override class func spec() {
        var viewModel: <Prefix>ViewModel!
        var mockAction: <Prefix>ViewModelActionMock!
        var dependency: <Prefix>ViewModelDependency!

        beforeEach {
            dependency = <Prefix>ViewModelDependency(
                fetcher: <Prefix>FetcherMock(),
                validator: <Module>ValidatorMock())
            viewModel = <Prefix>ViewModel(dependency: dependency)
            mockAction = <Prefix>ViewModelActionMock()
            viewModel.action = mockAction
        }

        describe("onViewDidLoad") {
            it("pushes the default title so the field is never blank") {
...
```swift
final class <Prefix>ViewModelActionMock: <Prefix>ViewModelAction {
    private(set) var actualTitleText: String?
    func setTitleText(_ text: String) { actualTitleText = text }
}

final class <Prefix>FetcherMock: <Prefix>FetcherProtocol {
    func fetch(spec: <SomeSpec>,
               completion: @escaping (<SomeResult>) -> Void) {
        // load bundled JSON fixture, call completion synchronously
        completion(<SomeResult>.fromFixture("<fixtureName>"))
    }
}
```
...
When the feature has test cases (`test-cases-resolve` above), every approved case with Automation `quick` gets exactly one test, titled with its ID: `it("TC-003: …")`. List `manual` cases in the report instead of testing them. Coverage of the diff below still applies on top. Run standalone with two active intent files, skip test cases and say so instead of asking, so writing tests never waits on planning hygiene.
