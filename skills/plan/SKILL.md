...
> **Core behaviors:** Goal-driven, so every task gets a verifiable check. Surface assumptions. Simplicity first: don't invent tasks the spec doesn't need. See `/using-agent-skills`.
...
| Field | Contents |
|-------|----------|
| **ID** | T1, T2, … (stable, since deps reference them) |
...
Offer `plan-roaster` rather than auto-running it. If the user accepts, pass the task table as the agent prompt and fold its weakest-assumption finding back into the plan before build.
