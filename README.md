...
| [`gate-stale-context.js`](hooks/gate-stale-context.js) | `PreToolUse` on `Edit\|Write\|MultiEdit\|NotebookEdit` | Editing a file that changed *after* `docs/context.md` was written, and that the doc describes, returns `ask`. Keyed on mtime rather than git, because a context doc is generated from a dirty worktree and diffing against the commit it records would re-flag every file already summarized |
