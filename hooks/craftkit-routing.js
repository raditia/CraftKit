...
process.stdin.on('end', () => {
  ...
  const context =
        ...
        "Skills (RN/web EVPMR): /fe-context /fe-scaffold /fe-review /fe-patterns /fe-a11y /fe-design /fe-performance /fe-test /code-quality /debug /ideate /think /ponytail-review /ponytail-audit /ponytail-debt\n" +
        "Planning (opt-in): /define chains interview→spec→test-cases→plan (checkpoint-gated), use for 'plan this feature' / 'spec before building'. Or invoke a phase alone: /interview (de-fuzz) · /spec (PRD) · /test-cases (QA cases from Figma/Lark, not test code) · /plan (tasks) · /adr (decision record) · /docs (dual-audience humanized docs). adr+docs also offered as tail of /parallel-ship. Never auto-run from /parallel-build.\n" +
        "General: /grill (stress-test existing plan/decision via frontier-round interview; 'grill'/'poke holes'/'stress-test') · /research (primary-source research note, background agent) · /handoff (compact session into handoff doc for fresh agent) · /eval (score a finished run into a weighted correctness %; 'score this run'/'how correct was that'/'success rate')\n" +
