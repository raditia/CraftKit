...
New always-active rule `rules/flag-safety.md`, unscoped so it loads on RN/web, Android, and iOS alike, since every platform gates features and the reasoning does not change. The bar is that flag OFF produces behavior observably identical to before the change, which makes flag state the only variable. It names the four surfaces where that actually breaks: code paths and shared helpers (a shared util both branches call is the real hazard, not the branch), persisted and cached state (writes made while ON must survive a flip back OFF mid-session), API request/response contracts (both directions hold at once, because a client flip is instant and a deploy is not), and analytics events (renaming one breaks dashboards during rollout). Plus the patterns that hold the bar: default to old behavior when a flag read fails, gate once at the boundary rather than in eight leaves, keep the old code reachable instead of re-implementing it, and test both states.

Enforced on both sides, the way the ponytail rubric is. Write time: a flag self-pass in the implement gate of `/build` and `/parallel-build`, and in the `/fix` verification gate, since a fix that only holds with the flag ON leaves the rollback broken. Read time: `flag-safety` is spliced into `fe-review`, `android-review`, and `ios-review` via `craftkitInject`, so the cold agents carry the live text rather than a copy that rots; `code-quality` gets a prompt addition when the diff reads a flag, since it is the platform-agnostic reviewer; `adversarial` gets rollback traps as an explicit hunting target; and `/parallel-ship` adds a rollback gate plus a `Rollback:` report line, where an unverified OFF path or a missing marker blocks rather than suggests.

Each branch carries a `flag:` comment naming the key, the OFF behavior, and the removal condition. That marker resolves the conflict it would otherwise create with ponytail: the protected list in `karpathy-guidelines` rule 2 now covers `flag:` alongside `ponytail:`, so a live rollback path is never scored as dead config, and the three ponytail skills inherit it from that single source. `/ponytail-debt` reads both markers now, reporting live flags and tagging `[removable]` where a removal condition already holds, because a marker with an exit condition and nothing that reads it is how flag debt becomes permanent branching.
...
- **`gate-announce-honored.js` (`Stop`).** Matches `Running /<name>` at line start in the turn's own assistant text against the `Skill` calls the transcript actually recorded, and blocks the stop on any name announced but not invoked. Three narrowings keep it from blocking its own documentation: the match is line-anchored (a backticked mid-sentence mention passes), fenced blocks are stripped first, and a name is only enforced if it resolves to an installed skill or command. Unresolvable names fail open, which includes plugin skills (`<plugin>:<name>`), marked `ponytail:` with the upgrade.
- **`craftkit-transcript.js` returns two more fields.** `assistantText` (what the agent said this turn, where an announcement lives) and `slashCommands` (the names the user typed, so a command you invoked yourself counts as invoked). The turn boundary stays defined in exactly one place, which is the point of the shared scanner.
- **`craftkit-routing.js` enumerates locally installed skills.** Anything under `~/.claude/skills/` or `<project>/.claude/skills/` belongs to the user, not craftkit, so it can never appear in the hardcoded table, and therefore never entered the classification set at all. `/humanizer` was invisible to routing for that reason, which is the second half of why it was skipped rather than merely unrun. The hook now lists whatever the table did not already name. Enumeration stats through symlinks: marketplace installers place a skill as a link, so `withFileTypes().isDirectory()` reported nothing, which is how the first cut of this silently found zero skills on the machine that needed it.
- **`check.sh` check 23 covers the new gate behaviorally**, seven cases: the lie blocks, an honest turn does not, a backticked mention does not, a fenced example does not, an uninstalled name does not, `stop_hook_active` does not re-block, and an unreadable transcript fails open. The fixture skill is created under the fixture's own `.claude/`, so the case does not depend on what the machine running the gate happens to have installed.
