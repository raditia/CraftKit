...
- **Android** (`*.kt`/`*.java`, Gradle) → tests via `/android-test` + `./gradlew :<module>:testGeneralDebugUnitTest`; type/lint gate = `./gradlew :<module>:lintGeneralDebug`; review = `/android-review`. **Skip the EVPMR/tsc/jest gates below** (no fixed 93% coverage bar unless the team sets one, so report the module's actual coverage).
