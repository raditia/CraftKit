...
**Model:** cheapest (Claude Code: the injected cheapest tier · Gemini `flash-lite` · Cursor `auto` · Codex default at effort `low`). Escalate to everyday only if diff spans > 15 files with non-obvious interdependencies.
