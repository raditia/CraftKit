...
| `partials/*.md` | Only spliced into a host file | Never directly | shared body pulled in via `craftkitInject:` by a skill, command or agent; syncs to no tool on its own, so it costs nothing resident |
...
- **`craftkitInject`: live rule/skill/partial injection into skills, cold agents and commands.** A skill, agent or command whose frontmatter has `craftkitInject: ruleA, skillB` gets those bodies (frontmatter stripped) spliced in as a managed block (`<!-- BEGIN CRAFTKIT-INJECTED-RULES ... END -->`) right after its own frontmatter, at install time. Each name resolves `partials/<name>.md` first, then `rules/<name>.md`, then `skills/<name>/SKILL.md`, so a file can carry a live partial (`parallel-review` ← `partials/parallel-classifier`), a live rule (`fe-review` ← `fe-rules`), or a live skill checklist (`android-review` ← `skills/android-review`). This is the escape hatch for the cold-copy problem below: the agent carries the *live* text, regenerated on every sync, instead of a hand-maintained duplicate. **The renderer is tool-agnostic and lives in `sync.sh` (`craftkit_render_injected`), not in an adapter.** Agents render only through Claude, the one tool with cold sub-agents, but all four adapters install the same `SKILL.md` and the same commands, so an adapter-local splice would ship Cursor, Gemini and Codex a skill with its core section silently absent. `check.sh` check 30 holds this: the renderer splices (behavioral), and `sync_adapter` and `sync_commands_adapter` both call it and install the rendered file. Reference impl: `sync.sh:craftkit_render_injected` (+ `craftkit_inject_list`, `craftkit_strip_frontmatter`), with `claude.sh:_claude_render_injected` a thin delegate beside `effective_claude_agent_source`. Commands render in `sync_commands_adapter` for every adapter (v1.46.0): before that they rendered only through Claude, so Cursor, Gemini and Codex installed commands naming partials they did not carry. An unresolvable name warns to stderr and is skipped; no opt-in field → plain copy. Reference consumers: `agents/fe-review.md` (rule), `agents/android-review.md` (skill), `skills/fe-patterns` + `agents/fe-patterns` (one partial shared by a skill and its cold agent). Runtime injection via a `SubagentStart` hook is **not** possible: that event is display-only and cannot add `additionalContext` to a subagent; sync-time splicing is the only mechanism.
...
- **Plain punctuation, no em-dashes.** The em-dash is a recognizable machine-writing tell, so this repo's prose reaches for a comma, colon, semicolon, period, or parentheses instead, picked per sentence rather than substituted mechanically. `check.sh` gates it. Two classes keep theirs because they are wire format, not prose, and each is exempted by the gate: the `## vX.Y.Z — date` CHANGELOG heading (`CHANGELOG.md` is a historical record and stays untouched), and the `CRAFTKIT-INJECTED-RULES` marker, which is written into installed agent files. Quoting upstream text is **not** exempt, so normalize the separator when citing a source that uses em-dashes, and say in the note that you did. The main managed-block marker no longer needs an exemption: v1.29.1 renamed it from `AGENTIC-SKILLS` to `CRAFTKIT` and dropped the em-dash, migrating the block already on disk first so no adapter could lose track of it.
