...
| `partials/*.md` | Only spliced into a host file | Never directly | shared body pulled in via `craftkitInject:`; syncs to no tool on its own, so it costs nothing resident |
...
- **`craftkitInject`: live rule/skill/partial injection into cold agents and commands.** An agent or command whose frontmatter has `craftkitInject: ruleA, skillB` gets those bodies (frontmatter stripped) spliced in as a managed block (`<!-- BEGIN CRAFTKIT-INJECTED-RULES ... END -->`) right after its own frontmatter, at install time. Each name resolves `partials/<name>.md` first, then `rules/<name>.md`, then `skills/<name>/SKILL.md`, so a file can carry a live partial (`parallel-review` ← `partials/parallel-classifier`), a live rule (`fe-review` ← `fe-rules`), or a live skill checklist (`android-review` ← `skills/android-review`). This is the escape hatch for the cold-copy problem below: the agent carries the *live* text, regenerated on every sync, instead of a hand-maintained duplicate. Reference impl: `claude.sh:_claude_render_injected` (+ `_claude_inject_list`, `_claude_strip_frontmatter`, `effective_claude_agent_source`, `effective_claude_command_source`). Commands go through the same splice, so `sync_commands_adapter` consults `effective_<a>_command_source` exactly as the agent pass does: without it a command whose partial moved on looks unchanged and never re-syncs. An unresolvable name warns to stderr and is skipped; no opt-in field → plain copy. Reference consumers: `agents/fe-review.md` (rule), `agents/android-review.md` (skill). Runtime injection via a `SubagentStart` hook is **not** possible: that event is display-only and cannot add `additionalContext` to a subagent; sync-time splicing is the only mechanism.
