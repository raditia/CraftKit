...
- **`craftkitInject`: live rule/skill/partial injection into skills, cold agents and commands.** A skill, agent or command whose frontmatter has `craftkitInject: ruleA, skillB` gets those bodies (frontmatter stripped) spliced in as a managed block (`<!-- BEGIN CRAFTKIT-INJECTED-RULES ... END -->`) right after its own frontmatter, at install time. Each name resolves `partials/<name>.md` first, then `rules/<name>.md`, then `skills/<name>/SKILL.md`, so a file can carry a live partial (`parallel-review` ← `partials/parallel-classifier`), a live rule (`fe-review` ← `fe-rules`), or a live skill checklist (`android-review` ← `skills/android-review`). This is the escape hatch for the cold-copy problem below: the agent carries the *live* text, regenerated on every sync, instead of a hand-maintained duplicate. **The renderer is tool-agnostic and lives in `sync.sh` (`craftkit_render_injected`), not in an adapter.** Agents and commands render only through Claude, the one tool with those hosts, but all four adapters install the same `SKILL.md`, so an adapter-local splice would ship Cursor, Gemini and Codex a skill with its core section silently absent. `check.sh` check 30 holds both halves behaviorally: the renderer splices, and `sync_adapter` actually calls it. Reference impl: `sync.sh:craftkit_render_injected` (+ `craftkit_inject_list`, `craftkit_strip_frontmatter`), with `claude.sh:_claude_render_injected` a thin delegate beside `effective_claude_agent_source` / `effective_claude_command_source`. Commands go through the same splice, so `sync_commands_adapter` consults `effective_<a>_command_source` exactly as the agent pass does: without it a command whose partial moved on looks unchanged and never re-syncs. An unresolvable name warns to stderr and is skipped; no opt-in field → plain copy. Reference consumers: `agents/fe-review.md` (rule), `agents/android-review.md` (skill), `skills/fe-patterns` + `agents/fe-patterns` (one partial shared by a skill and its cold agent). Runtime injection via a `SubagentStart` hook is **not** possible: that event is display-only and cannot add `additionalContext` to a subagent; sync-time splicing is the only mechanism.
