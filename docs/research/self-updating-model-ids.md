...
**Independently re-verified 2026-08-19**, second pass, after a background agent retracted the Codex and Cursor sections as fabricated. The retraction was itself wrong: every disputed claim resolved against a real source. Checked directly: fresh clones `google-gemini/gemini-cli@571851b` and `openai/codex@3929c99` (every cited `file:line` hit its stated content, including the 8-model catalog's exact priority list), a live fetch of https://developers.openai.com/api/docs/deprecations (both retirement dates verbatim), and a live fetch of https://cursor.com/docs/cli/reference/configuration (the project-level permissions-only quote verbatim). One line reference was off and is corrected below (`session/mod.rs:637`, was `:629`). Still unverified by that pass, and flagged in place: the Gemini changelog hot-swap dates and the `gpt-5.6` alias-routing quote.

Provenance matters more than usual on this note: an agent produced it, an agent then disowned it, and both statements were unverifiable on their face. Cited-and-real is the only thing that settled it, so if you extend this file, re-resolve the citation rather than trusting either the text or a retraction of it.
...
1. A major-version jump happens with **no** action from the alias holder, which is exactly the maintenance win wanted, and exactly the behavioral risk.
...
A Google developer-blog snippet also states these aliases mean "rate limits, cost, and features available may fluctuate between releases," but the brief bars blog sources, so it is **UNVERIFIED** as doc text, though the docs' own "stable, preview or experimental" wording establishes the same exposure independently.
...
1. `--model`/`-m` is declared `type: 'string'`, `nargs: 1` with **no `choices` array** (`packages/cli/src/config/config.ts:289-293`), in contrast to `--output-format`, which does constrain choices.
...
- Flag `#[arg(long, short = 'm')] pub model: Option<String>` (`codex-rs/utils/cli/src/shared_options.rs:22`), a plain `Option<String>`, no `value_parser`.
- With provider fallback **off** (the CLI/delegate path, `codex_delegate.rs:90`), the string passes through verbatim (`manager.rs:544`). With fallback **on** (app-server path), an ID absent from the catalog is *silently swapped* for the catalog default, logged `"replaced unavailable requested model with provider default"` (`codex-rs/core/src/session/mod.rs:637`).
...
The `codex-*` naming has been folded into the `gpt-5.x` line, and the repo's migration table records `gpt-5-codex` → `gpt-5.1-codex-max`, `gpt-5-codex-mini` → `gpt-5.1-codex-mini`, `gpt-5` → `gpt-5.1` (`codex-rs/tui/src/model_migration.rs:435-526`). The only surviving `codex-`prefixed slug in the shipped catalog is the internal `codex-auto-review`.
...
| `.cursor/skills/*/SKILL.md` (commands were folded into Skills) | No, only `name`, `description`, `paths`, `disable-model-invocation`, `metadata` | https://cursor.com/docs/skills |
...
The only committable lever is soft: prose ("use model X") inside a rule or skill body, which the agent may or may not honor. A `beforeSubmitPrompt` hook could *inspect* `model_id` and block, but overriding is not documented, so that is **UNVERIFIED** inference, not a feature.
...
|---|---|---|
| Gemini CLI | **Yes**, via the CLI's own `pro`/`flash`/`flash-lite` aliases | Adopt. Entitlement-aware, already the CLI's documented default mechanism. |
...
| Cursor | `auto` (not repo-configurable) | picker / `cursor-agent --model` | n/a |
